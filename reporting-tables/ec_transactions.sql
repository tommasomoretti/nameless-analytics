CREATE OR REPLACE TABLE FUNCTION `tom-moretti.nameless_analytics.ec_transactions`(start_date DATE, end_date DATE) AS (
  ...
  transaction_logic as (
    select
      ...
      if(event_name = 'purchase', ifnull(safe_cast(json_value(ecommerce, '$.value') as float64), 0.0), null) as purchase_revenue,
      if(event_name = 'purchase', ifnull(safe_cast(json_value(ecommerce, '$.shipping') as float64), 0.0), null) as purchase_shipping,
      if(event_name = 'purchase', ifnull(safe_cast(json_value(ecommerce, '$.tax') as float64), 0.0), null) as purchase_tax,
      
      if(event_name = 'refund', ifnull(safe_cast(json_value(ecommerce, '$.value') as float64), 0.0), null) as refund_revenue,
      if(event_name = 'refund', ifnull(safe_cast(json_value(ecommerce, '$.shipping') as float64), 0.0), null) as refund_shipping,
      if(event_name = 'refund', ifnull(safe_cast(json_value(ecommerce, '$.tax') as float64), 0.0), null) as refund_tax
    from base_events
  ),
