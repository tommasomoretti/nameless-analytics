...
- If no nameless_analytics_user cookie or nameless_analytics_session are present in the request, the client tag generates two values (one for nameless_analytics_user cookie and one for nameless_analytics_session cookie), adds these values as event parameters and sets two cookies with the response.
